...
* :doc:`/howto/trigger-rules` - set the conditions under which a DAG will run a task
...
However, this is just the default behaviour, and you can control it using the ``trigger_rule`` argument to a Task. Please see main article :doc:`/howto/trigger-rules` for details about the available options.
